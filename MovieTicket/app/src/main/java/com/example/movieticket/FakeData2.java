package com.example.movieticket;

import com.example.movieticket.models.TicketMovie;

import java.util.List;

public class FakeData2 {
    static String poster = "https://m.media-amazon.com/images/I/71niXI3lxlL._AC_SY679_.jpg";
    public static List<TicketMovie> recommend = List.of(
            new TicketMovie("Spiderman", poster, "Action, Adventure, Fantasy", "2018", "523545", "138 minnutees"),
            new TicketMovie("Spiderman", poster, "Action, Adventure, Fantasy", "2018", "513545", "138 minnutees"),
            new TicketMovie("Spiderman", poster, "Action, Adventure, Fantasy", "2018", "123545", "138 minnutees"),
            new TicketMovie("Spiderman", poster, "Action, Adventure, Fantasy", "2018", "512345", "138 minnutees"),
            new TicketMovie("Spiderman", poster, "Action, Adventure, Fantasy", "2018", "5123545", "138 minnutees"),
            new TicketMovie("Spiderman", poster, "Action, Adventure, Fantasy", "2018", "512354", "138 minnutees"));
}
